...
router.get('/maps/property/:id', authenticate, async (req, res, next) => { try { const [rows] = await pool.query(`${mapsBaseQuery} WHERE l.lot_id = ?`, [req.params.id]); if (rows[0]) res.json(rows[0]); else res.status(404).json({ message: 'Property not found' }) } catch (e) { next(e) } })
