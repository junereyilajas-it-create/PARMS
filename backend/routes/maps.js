...
router.get('/maps/properties', authenticate, allowRoles('admin', 'assessor', 'staff'), async (_, res, next) => { try { const [rows] = await pool.query(`${mapsBaseQuery} WHERE g.latitude IS NOT NULL AND g.longitude IS NOT NULL`); res.json(rows) } catch (e) { next(e) } })
...
router.get('/maps/barangay/:barangay', authenticate, allowRoles('admin', 'assessor', 'staff'), async (req, res, next) => { try { const [rows] = await pool.query(`${mapsBaseQuery} WHERE br.barangay_name LIKE ? AND g.latitude IS NOT NULL`, [`%${req.params.barangay}%`]); res.json(rows) } catch (e) { next(e) } })
